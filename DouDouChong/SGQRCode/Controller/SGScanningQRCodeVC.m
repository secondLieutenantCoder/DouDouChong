...
@property (nonatomic) CLLocationCoordinate2D userPoint;
...
- (void)readImageFromAlbum {
    ...
    if (device) {
        ...
        if (status == PHAuthorizationStatusNotDetermined) { // 用户还没有做出选择
            ...
            SGAlertView *alertView = [SGAlertView alertViewWithTitle:@"⚠️ 警告" delegate:nil contentTitle:@"请去-> [设置 - 隐私 - 照片 - 益善通] 打开访问开关" alertViewBottomViewType:(SGAlertViewBottomViewTypeOne)];
            [alertView show];
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputMetadataObjects:(NSArray *)metadataObjects fromConnection:(AVCaptureConnection *)connection {
    ...
    // 3、设置界面显示扫描结果
    //http://www.cishan123.org/QRCode/功能标识?id=
#pragma mark 二维码扫描结果判断
    if (metadataObjects.count > 0) {
        AVMetadataMachineReadableCodeObject *obj = metadataObjects[0];
        
        NSLog(@"metadataObjects二维码内容 = %@", metadataObjects);
        NSLog(@"objStringValue = %@",obj.stringValue);
        
        if ([obj.stringValue hasPrefix:@"http://www.cishan123.org/QRCode/"]) {
//            NSCharacterSet *set=[NSCharacterSet characterSetWithCharactersInString:@"?/"];
//            NSArray *array=[obj.stringValue componentsSeparatedByCharactersInSet:set];
//            if ([array[4] isEqualToString:@"ActivitySignIn"]) {
//                [self respond];
//            }
            
        }else if ([self deptNumInputShouldNumber:obj.stringValue]){
//            obj.stringValue.length == 18 &&
            /**
             * 之前判断18位
             * 新需求 是数字串就行
             *wangwei
             ...
             */
        // 扫描义工证获取到的义工编码
//            [self respondCerIDWithNumber:obj.stringValue];
            
        } else { // 扫描结果为条形码
            /// 如果是调用外部浏览器打开URL网页，益善通退出二维码扫描页，回到主页；
//            [self.navigationController popViewControllerAnimated:YES];
//            ScanSuccessJumpVC *jumpVC = [[ScanSuccessJumpVC alloc] init];
...
#pragma mark - 扫描义工证的响应
- (void) respondCerIDWithNumber:(NSString *) cerNumber{

//    NSString *strJD = [NSString stringWithFormat:@"%lf",jd];
//    NSString *strWD = [NSString stringWithFormat:@"%lf",wd];
//    
//    NSString *path=[NSString stringWithFormat:CODE,URL_HOST1];
//    if (wd==0.0&&jd==0.0) {
////        path=[NSString stringWithFormat:CODE1,URL_HOST1,accountId];
...
//    //    ,accountId,strJD,strWD
//    NSDictionary * param = @{@"SerialNumber":cerNumber,
//                             @"AccountID":@"",
//                             @"JD":strJD,
//                             @"WD":strWD
//                             };
...
//    
//    [manager POST:path1 parameters:param success:^(NSURLSessionDataTask *task, id responseObject) {
//        NSDictionary *dic=[NSJSONSerialization JSONObjectWithData:responseObject options:NSJSONReadingMutableContainers error:nil];
//        NSNumber *num=[dic objectForKey:@"resultCode"];
//        if (num.integerValue==0) {
//            [self alertWithTitle:[dic objectForKey:@"resultMessage"]];
//        }else{
//            [self.navigationController popViewControllerAnimated:YES];
//        }
//        
...
{
//    NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
//    NSDictionary *dic=[defaults objectForKey:@"PersonInfor"];
//    
//    NSString *accountId = [dic objectForKey:@"accountId"];
//    NSString *strJD = [NSString stringWithFormat:@"%lf",jd];
//    NSString *strWD = [NSString stringWithFormat:@"%lf",wd];
//    
//    // VolunteerSignWithactivityId
////    NSString * actID = [defaults objectForKey:<#(nonnull NSString *)#>];
////    HttpRequest VolunteerSignWithactivityId:account andaccountId:<#(NSString *)#> andStrJD:<#(NSString *)#> andStrWD:<#(NSString *)#> andSuccess:<#^(id obj, NSError *err)success#>
//    
//    NSString *path=[NSString stringWithFormat:CODE,URL_HOST1];
//    
//    if (wd==0.0&&jd==0.0) {
...
//{
//    wd=userLocation.location.coordinate.latitude;//纬度
//    jd=userLocation.location.coordinate.longitude;//jingdu度
//
